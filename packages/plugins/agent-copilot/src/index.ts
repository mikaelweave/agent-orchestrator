...
/**
 * Section appended to AGENTS.md as a secondary signal for Copilot.
 */
...
async function setupCopilotWorkspace(workspacePath: string): Promise<void> {
  ...
  return {
    ...
    getLaunchCommand(config: AgentLaunchConfig): string {
      ...
      if (config.model) {
        ...
        // claude-3.5-sonnet, gemini-2.0-flash, etc.
    ...
    detectActivity(terminalOutput: string): ActivityState {
      ...
      // If Copilot is showing its input prompt or shell prompt, it's idle
      if (/^[>$#?]\s*$/.test(lastLine)) return "idle";

      // Check last few lines for approval prompts
      const tail = lines.slice(-5).join("\n");
      if (/\? What would you like to/i.test(tail)) return "waiting_input";
      if (/\(y\)es.*\(n\)o/i.test(tail)) return "waiting_input";
      if (/Press Enter/i.test(tail)) return "waiting_input";

      // Default to active — the agent is processing or showing output
      return "active";
    },
    ...
    async getActivityState(
      ...
      // NOTE: GitHub Copilot CLI does not maintain per-workspace session files
      // that can be reliably introspected. We return null (unknown) rather than
      // guessing from terminal output alone.
      return null;
    },
    ...
    async getSessionInfo(_session: Session): Promise<AgentSessionInfo | null> {
      // GitHub Copilot CLI does not expose per-session JSONL or structured logs
      // for introspection. Cost and summary data are not available.
      return null;
    },

    async setupWorkspaceHooks(workspacePath: string, _config: WorkspaceHooksConfig): Promise<void> {
      await setupCopilotWorkspace(workspacePath);
    },

    async postLaunchSetup(session: Session): Promise<void> {
      if (!session.workspacePath) return;
      await setupCopilotWorkspace(session.workspacePath);
    },
  };
}
